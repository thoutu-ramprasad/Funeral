...
<h2 class="text-left f-24 l-36 ls-minus-2 color-white res-summary"> 
	<button type="button" class="p-0 text-left f-22 l-36 ls-minus-2 color-white res-summary btn btn-link text-decoration-none" data-toggle="collapse" data-target="#collapseThree" style="text-decoration:none;">Your policy summary</button>
</h2>
</div>
...
<div id="collapseThree" class="col-12 collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
                <div class="col-12 text-left card-body p-1">
                    <p>
						<div class="col-12 bg-gray pl-20 pt-10 pb-2 mb-20 text-left">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-6 mt-4">
                                                    <div class="single_input">
                                                        ...
                                                        <div class="mt-10 wide">
                                                            <label class="left f-12 l-17 m-0 color-grey mt-4">Full name</label><input type="text" id="c_firstname" name="c_firstname" autocomplete="off" placeholder="Full Name" onFocus="this.placeholder = ''" onBlur="this.placeholder = 'FullName'"  class="single-input p-0 transparent-input" value="<?php echo isset($client_firstname) ? $client_firstname : ''; ?>">
                                                        </div>
...
$('.three_btn').click(function(){
    var ctitle = jQuery('#c_title').val();
    var bfirstname = jQuery('#b_firstname').val();
    var cfirstname = jQuery('#c_firstname').val();
    var csurname = jQuery('#c_surname').val();
    var cidnumber = jQuery('#c_idnumber').val();
    var cmaritalstatus = jQuery('#c_marital').val();
    var cphonenumber = jQuery('#c_contactnumber').val();
    var cemail = jQuery('#c_email').val();
    var caddress = jQuery('#c_address').val();
    var bsurname = jQuery('#b_surname').val();
    var bidnumber = jQuery('#b_idnumber').val();
    var bphonenumber = jQuery('#b_contactnumber').val();
    var bemail = jQuery('#b_email').val();
    var baltcontactnumber = jQuery('#b_altcontactnumber').val();
    var brelation= jQuery('#b_relationship').val();

        var err = [];
        var cnt = 0;
            
        if (ctitle == '') {
          $("body#step3").addAttr('disabled','disabled');
            $(".error_ctitle").html("Select the relation.").css('color', 'red');
            err += cnt + 1;
        } else {
            $(".error_ctitle").html("");
        }
        if (cfirstname == '') {
            $(".error_cfname").html("Fill the first name.").css('color', 'red');
            err += cnt + 1;
        } else {
            $(".error_cfname").html("");
        }
        if (csurname == '') {
            $(".error_csurname").html("Fill the surname.").css('color', 'red');
            err += cnt + 1;
        } else {
            $(".error_csurname").html("");
        }
        if (cidnumber == '') {
            $(".error_cidnumber").html("Fill the idnumber.").css('color', 'red');
            err += cnt + 1;
        } else {
            $(".error_cidnumber").html("");
        }
        
        if (cmaritalstatus == '') {
            $(".error_cmaritalstatus").html("Select the status.").css('color', 'red');
            err += cnt + 1;
        } else {
            $(".error_cmaritalstatus").html("");
        }
        if (cphonenumber == '') {
            $(".error_cphonenumber").html("Fill the cell number.").css('color', 'red');
            err += cnt + 1;
        } else {
            $(".error_cphonenumber").html("");
        }
        var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
		if(cemail == ""){
				$('#error_cemail').html('Fill the email');
				er+=cnt+1;
			}else if(! emailReg.test(cemail)){
				$('#error_cemail').html('Please enter required email id');
				er+=cnt+1;
			}else{
				$('#error_cemail').html('');
			}
        ...
        if (caddress == '') {
            $(".error_caddress").html("Fill the address.").css('color', 'red');
            err += cnt + 1;
        } else {
            $(".error_caddress").html("");
        }
        ...
        if (brelation == '') {
           ...
           $(".error_brelation").html("Select the relation.").css('color', 'red');
           err += cnt + 1;
       } else {
           $(".error_brelation").html("");
       }
       if (bfirstname == '') {
           $(".error_bfname").html("Fill the first name.").css('color', 'red');
           err += cnt + 1;
       } else {
           $(".error_bfname").html("");
       }
       if (bsurname == '') {
           $(".error_bsurname").html("Fill the surname.").css('color', 'red');
           err += cnt + 1;
       } else {
           $(".error_bsurname").html("");
       }
       if (bidnumber == '') {
           $(".error_bidnumber").html("Fill the idnumber.").css('color', 'red');
           err += cnt + 1;
       } else {
           $(".error_bidnumber").html("");
       }
       
       
       if (bphonenumber == '') {
           $(".error_bphonenumber").html("Fill the cell number.").css('color', 'red');
           err += cnt + 1;
       } else {
           $(".error_bphonenumber").html("");
       }
       if (baltcontactnumber == '') {
           $(".error_baltphonenumber").html("Fill the alternate cell number.").css('color', 'red');
           err += cnt + 1;
       } else {
           $(".error_baltphonenumber").html("");
       }
       
       if(bemail == ""){
				$('#error_bemail').html('Fill the email');
				er+=cnt+1;
			}else if(! emailReg.test(bemail)){
				$('#error_bemail').html('Please enter required email id');
				er+=cnt+1;
			}else{
				$('#error_bemail').html('');
			}
